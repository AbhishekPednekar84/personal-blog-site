...
In the previous example, we fixed a "bad" commit message. Now, what if we forgot to include an entire file before running our `git commit`? Let's say, we now create a .gitignore file in our repository and would like to include it with the **last** commit that we made. Once we add the files to staging, we can simply run `git commit --amend`. Running this command opens an interactive window that shows us the changes that will be committed to the last commit. The commit message can also be modified if needed. Since we are not making any further changes, we can use `:wq` to exit. Following this, our .gitignore file will be added to the last commit.
